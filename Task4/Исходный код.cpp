...
void Deck::double_capacity() {
	...
	// Заполняем новый буфер от головы дека до хвоста
...
void Deck::push_back(int value) {
	// Заталкиваем значение в хвост, если достигнут конец массива, переходим на его начало
	++tail %= capacity;
	buffer[tail] = value;
	size++;
...
void Deck::push_front(int value) {
	// Если голова дека находится в начале массива, то переносим ее в конец, иначе идем влево по массиву
	if(head == 0)
		head = capacity - 1;
	else
		head--;
